...
TTY で起動すると全画面 TUI（ステータスバー + シーン本文 + 選択肢）でプレイでき、
パイプ・リダイレクトなどの非 TTY 環境では自動的に行ベース表示にフォールバックします。
...
ステータスバーには全 Ink 変数が `name = value` 形式で表示され、`:set` や進行で即座に更新されます。
`:vars` で非表示にするとシーン本文の領域が広がります。

シナリオが終端に達すると「物語は終わりを迎えた」と表示され、`Enter` または `q` で終了します。

## 操作（行ベース / 非 TTY・`--plain`）

`入力>` プロンプトに対し、選択肢の番号、または `:get` / `:set` / `:quit` を 1 行で入力します。
