...
//   - 固定コマンド（あたりをみろ/しらべろ/きけ/なにかとれ/うごけ/もちもの/つきつけろ）
...
describe("detective_command smoke play (#12 / 探偵コマンド)", () => {
  ...
  it("別人（ボウ）に突きつけても当てはまらず、進行不能にならない", () => {
    gatherDecisive(engine);
    choose(engine, "つきつけろ");
    const wrong = choose(engine, "ボウ");
    expect(wrong.text).toContain("首を振った");
    expect(engine.getVariable("has_key")).toBe(false);
