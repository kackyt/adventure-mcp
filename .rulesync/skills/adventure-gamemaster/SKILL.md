...
1. **開始**
   - `list_scenarios` を呼び、id 一覧をユーザーに提示して選んでもらう。
   - 選ばれた id で `start_game` を呼ぶ。返ってきた `sessionId` を以後すべての呼び出しで使う。
2. **状況の演出**
   - `scene` を GM の口調で語る。`status`（例: HP・所持金）が空でなければ自然に織り交ぜる。
   - `choices` を 1) 2) … と番号付きで提示する（番号はツールの `index` に対応）。
3. **行動の解決**
   - ユーザーの自然言語の入力を、**提示済み `choices` のどれか**に対応付ける。
   - `choose(sessionId, index, expectedText)` を呼ぶ。`expectedText` には対応付けた選択肢の
     **ラベル文字列**を渡す（誤対応を engine 側で弾く保険）。
