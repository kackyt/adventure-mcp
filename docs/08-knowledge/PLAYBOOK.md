...
### ACE-16-1: セキュリティとアーキテクチャ境界の自動テスト
...
**Insight**: 外部向けのインターフェースがコアの依存モジュールを直接インポートしていないか、アンチチート等の観点からテストで担保する。

**Context**: AI向けのMCPサーバー（`mcp-server`）が、ゲーム進行エンジンである `inkjs` を直接インポートして内部ステータスを操作（チート）できないよう、テストで `src` 配下および `package.json` のインポートを検証し、構造的に防いだ。

**Action**: コアロジックの依存を隠蔽するアーキテクチャ（ヘキサゴナル等）を採用した場合、外層が内層の依存ライブラリを直接使っていないことを静的解析やテストで継続的に検証する。

<a id="ace-16-2"></a>

### ACE-16-2: 生成AI向けの公開ステータスフィルタリング
...
**Insight**: 生成AIにコンテキストを渡す際、内部の全変数を渡すと非公開情報（解法フラグなど）のメタ推理を引き起こすため、公開用にホワイトリスト化されたステータスのみを渡す。

**Context**: Inkシナリオの進行をAIに任せるMCPサーバーにおいて、作者が公開を意図した変数（`public_status`）のみを抽出し、AIにはそれ以外の内部変数を隠蔽する設計とした。

**Action**: AIに状態を共有する際は、内部状態をそのままダンプせず、公開可能な情報のみを明示的にフィルタリングする層を設ける。

<a id="ace-16-3"></a>

### ACE-16-3: テキスト照合時の空文字ガード処理
...
**Insight**: 期待値となる文字列と実際の値を照合（部分一致等）する際、期待値が空文字や空白のみの場合に意図しない挙動や照合失敗が起こるのを防ぐガード処理を入れる。
