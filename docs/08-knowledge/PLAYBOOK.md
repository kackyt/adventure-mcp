...
### ACE-27-1: AIによる正誤判定の禁止と決定論的判定への委譲
...
**Insight**: 暗証番号や合言葉などの自由入力を受け付ける際、AIに正誤判断（曖昧マッチや言い換え）をさせず、入力値をそのままゲームエンジンに渡して決定論的な完全一致判定を行わせる。

**Context**: PR #27にて、AIが「惜しい回答」を正解扱いしてしまう構造的問題を排除するため、`submit_input` ツールを追加し、AIに対して正誤判断・言い換え・補正を禁止するようシステムプロンプト/ツール説明で制約した。

**Action**: MCPツールの説明文などでAIによる入力評価を明示的に禁止し、入力値は正規化（NFKC等）のみを行ってバックエンドのシナリオエンジンへ素通しし、判定ロジックはバックエンド側に隠蔽する。

<a id="ace-27-2"></a>

### ACE-27-2: 複数フロントエンド間での特殊入力・空入力処理の統一
...
**Insight**: 複数のUI（CLIとTUIなど）を提供する際、デバッグコマンドへの切り替えキー（`:`等）や空文字入力の扱いをフロントエンド間で統一しないと、デバッグ機能の欠損やシナリオ進行の乖離が生じる。

**Context**: PR #27のレビューにて、CLIでは空入力を無視して再入力を促しデバッグにも入れるのに対し、TUIでは `:` キーが入力として消費されデバッグに入れず、空入力が空文字列としてエンジンに送信されてしまうという一貫性の欠如が指摘された。

**Action**: 入力受け付けコンポーネントを実装・修正する際は、UI層に関わらず、デバッグ用キーストロークのインターセプトや空入力時の挙動（無視するか送信するかの仕様）を共通化し、テストで両系の挙動一致を担保する。

<a id="ace-27-3"></a>

### ACE-27-3: インメモリコンパイルによるテストの実ファイル依存排除
...
**Insight**: シナリオエンジンのテストにおいて、実ファイルに依存せず、テストコード内に記述した最小限のシナリオ文字列をインメモリでコンパイルしてテストフィクスチャとする。
